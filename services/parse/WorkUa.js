  ...
  async init(searchText) {
    ...
    while (true) {
      ...
      for await (const card of jobCards) {
        ...
        const date = trim.whiteSpaces($('.card.wordwrap p.cut-bottom-print.flex.flex-wrap.flex-align-center span.text-muted').text())
        const description = trim.whiteSpaces($(card).find('.add-top-xs').text())
    ...
    return {title: '<a href="https://www.work.ua/">WorkUa</a>', message: formatLinks}
  }
