...
class Djinni {
  ...
  async init(searchText) {
    const formattedSearchText = searchText.toLowerCase().trim().split(' ').join('+')
    ...
    while (true) {
      ...
      for await (const card of jobCards) {
        const title = trim.whiteSpaces($(card).find('.list-jobs__title a span').text())
        const link = $(card).find('a').attr('href')
        const responseLink = await fetch(`https://djinni.co${link}`)
        const bodyLink = await responseLink.text()
        $ = cheerio.load(bodyLink)
        const date = trim.whiteSpaces($('.col-sm-8.row-mobile-order-2 .profile-page-section.text-small p.text-muted').first().contents().filter(function () {
          return this.nodeType === 3
        }).text())
        const description = trim.whiteSpaces($(card).find('.list-jobs__description p').text())
    ...
    return {title: '<a href="https://djinni.co/">Djinni</a>', message: formatLinks}
  }
